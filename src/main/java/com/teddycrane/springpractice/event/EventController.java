...
public class EventController extends BaseController implements IEventController {
	...
	public Event createEvent(CreateEventRequest request) throws DuplicateItemException, BadRequestException {
		...
		try {
			if (request.getName() != null && (request.getStartDate().isPresent() || request.getEndDate().isPresent())) {
				return this.eventService.createEvent(request.getName(), request.getStartDate().get(),
						request.getEndDate().get());
			} else {
				System.out.println(
						"Invalid request body.  Unable to create an event without a name and at least one of [startDate, endDate]");
				throw new BadRequestException(
	...
	public Event deleteEvent(String id) throws BadRequestException, EventNotFoundException {
		...
		try {
			...
			throw new BadRequestException(
					String.format("Unable to handle id %s.  Please check the provided id and try again. ", id));
		} catch (EventNotFoundException e) {
			throw new EventNotFoundException(e.getMessage());
		}
	}
