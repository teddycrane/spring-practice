...
public class RaceController extends BaseController implements IRaceController {
  ...
  @Override
  public Race startRace(String raceId)
  ...
  @Override
  public Map<UUID, Integer> getResultsForRacer(String id)
    ...
    if (EnumHelpers.testEnumValue(RaceFilterType.class, type)) {
      try {
        ...
        if (filterType == RaceFilterType.CATEGORY && isValidCategory) {
          return this.raceService.filterRace(
              filterType, Either.right(Category.valueOf(value.toUpperCase())));
