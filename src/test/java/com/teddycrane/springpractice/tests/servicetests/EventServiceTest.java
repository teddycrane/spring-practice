...
public class EventServiceTest {
    ...
    public void setUp() {
        ...
        this.eventService = new EventService(this.eventRepository, this.raceRepository);
        ...
        when(this.eventRepository.findById(testId)).thenReturn(Optional.of(expected));
    }
    ...
    public void createEvent_shouldCreateEvent() {
        ...
        Event actual = this.eventService.createEvent("event name", new Date(), new Date());
    ...
    public void createEvent_shouldHandleNameCollisions() {
        when(this.eventRepository.findByName("name")).thenReturn(Optional.of(expected));

        assertThrows(DuplicateItemException.class,
                () -> this.eventService.createEvent("name", expected.getStartDate(), expected.getEndDate()));
    }
}
